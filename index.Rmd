...
### Chart 1: How a Age Relates To Hours Slept Per Day
...
average_sleep_expr <- sleep_data %>% 
  group_by(exper) %>% 
  summarize(avg_sleep = mean(sleep), na.rm = TRUE)


chart_3 <- ggplot(average_sleep_expr, aes(x = exper, y =   avg_sleep/60/7)) + 
...
As we can see as the experience goes up the amount hours works also goes up, but this change happens more near the end of the plot. As a persons gains experience so does the amount of hours they work. Through this we could say that as a person gains more experience and goes further in the industry they continue to work more and have a worse work life balance. But if we use our relation to graph one it tells us a differnet story. We know that age increase there is also a increase in experience, so if we look at those with a higher age in graph 1, we notice they actually get more sleep. Despite those who are older with more experience working more, they are sleeping more, showing how they have a good work life balance.
